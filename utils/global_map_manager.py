...
class GlobalMapManager(BaseMapManager):
    def __init__(
        ...
        # === NEW: Semantic Map Manager ===
        semantic_save_dir = os.path.join(cfg.map_save_path, "..", "semantic")
        self.semantic_map_manager = SemanticMapManager(save_dir=semantic_save_dir)

        # === NEW: Spatial Relation Graph ===
        spatial_save_dir = os.path.join(cfg.map_save_path, "..", "semantic")
        self.spatial_relation_graph = SpatialRelationGraph(save_dir=spatial_save_dir)
    ...
    def _update_semantic_outputs(self):
        """Update semantic map JSON and spatial relation graph after global map changes."""
        try:
    ...
    def process_observations(self, curr_observations: List[Observation]) -> None:
        ...
        if self.is_initialized == False:
            ...
            # === NEW: Update semantic outputs after initialization ===
            self._update_semantic_outputs()
            return
        ...
        # === NEW: Update semantic outputs after every global map update ===
        self._update_semantic_outputs()
    ...
    def save_map(self) -> None:
        ...
        # === NEW: Final save of semantic outputs ===
        self._update_semantic_outputs()
        logger.info("[GlobalMap] Semantic map and spatial graph saved.")
    ...
    def load_map(self) -> None:
        ...
        # === NEW: Update semantic outputs after loading ===
        self._update_semantic_outputs()
    ...
    def load_wall(self) -> None:
        ...
        # Set all z values to floor_height
        points[:, 2] = self.cfg.floor_height
    ...
    def calculate_global_path(
        ...
        # Add layout wall info to the total pcd
